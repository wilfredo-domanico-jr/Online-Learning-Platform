...
Not yet built: enrollment, payments/Stripe, quizzes (the lesson `type` exists but has no quiz-taking flow), reviews, and instructor payouts — see the phased roadmap in the project's plan history for what's next.
...
- **Migrations**: stock Laravel auth tables, Sanctum's `personal_access_tokens`, Spatie's permission tables, the social-fields migration, and the Phase 1 domain tables (`categories`, `courses`, `course_sections`, `lessons`, `lesson_video_details`, `lesson_articles`, `lesson_resources`, `instructor_applications`). No enrollment/payment/quiz-attempt/review tables exist yet.
...
- **Lesson content visibility**: `LessonResource` (`backend/app/Http/Resources/LessonResource.php`) only serializes video/article/attachment content when the requester owns the course, is an admin, or the lesson `is_previewable`. The controller sets this via `$request->attributes->set('can_view_locked_lesson_content', bool)` rather than threading a flag through nested resource collections — read that file's comment before changing the locking logic. Enrollment-based unlocking (for paying students) is a Phase 2 concern; right now "locked" just means "not previewable and not the owner."
...
- **Frontend structure** under `frontend/src/`:
    - `api/` — axios client (`client.js`) plus per-domain endpoint modules: `auth.js`, `catalog.js` (public browse/detail/curriculum), `instructor.js` (applications, course/section/lesson CRUD, content uploads), `admin.js` (application/course moderation queues).
    ...
    - `features/` — one folder per domain: `auth/`, `dashboard/`, `catalog/` (`CourseListPage`, `CourseDetailPage`), `instructor/` (`ApplyPage`, `InstructorCoursesPage`, `CourseEditorPage` — the curriculum builder with up/down-button reordering, no drag-and-drop library), `admin/` (`ApplicationsQueuePage`, `CourseModerationPage`).
